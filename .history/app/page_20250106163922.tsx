...
export default function Home() {
  ...
  useGSAP(
    () => {
      const lines = containerRef.current?.querySelectorAll(".line"); // Select all elements with the "line" class
      if (!lines || lines.length === 0) return; // Exit if no lines are found

      // Animate each line individually
      gsap.fromTo(
        lines, // Safely use the selected elements
